...
class Solution:
    ...
    示例 1：https://assets.leetcode-cn.com/aliyun-lc-upload/uploads/2020/03/08/frog_2.png
    ...
    示例 2：https://assets.leetcode-cn.com/aliyun-lc-upload/uploads/2020/03/08/frog_3.png
    ...
    def dfs(self, u: int, parent: int):
        parent_in = 1 if parent > 0 else 0
        cnum = len(self.adj[u]) - parent_in

        if self.reach[u] < self.t:
            for v in self.adj[u]:
                if self.reach[v] == -1:
                    self.reach[v] = self.reach[u] + 1
                    self.prob[v] = self.prob[u] / cnum
                    self.dfs(v, u)

        if u == self.target and (self.reach[u] == self.t or cnum == 0):
            self.ans = self.prob[u]
        elif cnum >= 1:
            self.prob[u] = 0
