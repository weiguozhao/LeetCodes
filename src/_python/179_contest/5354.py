...
class Solution:
    ...
    示例 2：https://assets.leetcode-cn.com/aliyun-lc-upload/uploads/2020/03/08/graph.png
    输入：n = 6, headID = 2, manager = [2,2,-1,2,2,2], informTime = [0,0,1,0,0,0]
    输出：1
    解释：id = 2 的员工是公司的总负责人，也是其他所有员工的直属负责人，他需要 1 分钟来通知所有员工。
    上图显示了公司员工的树结构。

    示例 3：https://assets.leetcode-cn.com/aliyun-lc-upload/uploads/2020/03/08/1730_example_3_5.PNG
    ...
    def numOfMinutes(self, n: int, headID: int, manager: List[int], informTime: List[int]) -> int:
        self.n = n
        header_manager = [[] for _ in range(n)]
        for man, header in enumerate(manager):
            if header == -1:
                continue
            header_manager[header].append(man)
        self.manager = header_manager
        self.informTime = informTime
        self.ans = 0
        self.dfs(headID, informTime[headID])
        return self.ans

    def dfs(self, headId: int, time: int):
        self.ans = max(self.ans, time)

        for manager in self.manager[headId]:
            self.dfs(manager, time + self.informTime[manager])
